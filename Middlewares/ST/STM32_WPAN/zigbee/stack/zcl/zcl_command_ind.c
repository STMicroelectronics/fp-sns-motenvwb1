...
zcl_persist_handle_set(struct ZbZclClusterT *clusterPtr, uint8_t *buf, uint16_t len)
{
    ...
    for (i = 0; i < len; ) {
        ...
        status = ZbZclAttrWrite(clusterPtr, NULL, attrId, attrData, attrLen, ZCL_ATTR_WRITE_FLAG_FORCE | ZCL_ATTR_WRITE_FLAG_PERSIST);
...
ZbZclHandleReadAttr(struct ZbZclClusterT *clusterPtr, struct ZbZclHeaderT *zclHdrPtr, struct ZbApsdeDataIndT *ind)
{
    ...
    respHeader.frameCtrl.direction = (zclHdrPtr->frameCtrl.direction == ZCL_DIRECTION_TO_SERVER) ? ZCL_DIRECTION_TO_CLIENT : ZCL_DIRECTION_TO_SERVER;
    ...
    while (true) {
        ...
        status = ZbZclAttrRead(clusterPtr, attributeId, &attrType, &buf[offset + 2U], clusterPtr->maxAsduLength - offset - 2U, false);
...
ZbZclHandleWriteAttr(struct ZbZclClusterT *clusterPtr, struct ZbZclHeaderT *zclHdrPtr, struct ZbApsdeDataIndT *ind)
{
    ...
    switch (cmd_id) {
        ...
        case ZCL_COMMAND_WRITE:
            ...
            hdr.frameCtrl.direction = (zclHdrPtr->frameCtrl.direction == ZCL_DIRECTION_TO_SERVER) ? ZCL_DIRECTION_TO_CLIENT : ZCL_DIRECTION_TO_SERVER;
        ...
        case ZCL_COMMAND_WRITE_NO_RESPONSE:
            ...
            while (true) {
                ...
                status = ZbZclAttrWrite(clusterPtr, &ind->src, attr_id, &ind->asdu[offset], ind->asduLength - offset, ZCL_ATTR_WRITE_FLAG_NORMAL);
                if (status != ZCL_STATUS_SUCCESS) {
                    num_write_errors++;
                }
...
ZbZclHandleWriteStruct(struct ZbZclClusterT *clusterPtr, struct ZbZclHeaderT *zclHdrPtr, struct ZbApsdeDataIndT *dataIndPtr)
{
    ...
    while (true) {
        ...
        status = ZbZclWriteElement(attr, attrType, (int)ZCL_ATTRIBUTE_BUFFER_SIZE_MAX, elem, (uint8_t)elem_type, elem_size, indicator, index);
        if (status == ZCL_STATUS_SUCCESS) {
            /* Successfully replaced the selected element, now try to write the attribute. */
            status = ZbZclAttrWrite(clusterPtr, &dataIndPtr->src, attr_id, attr, ZCL_ATTRIBUTE_BUFFER_SIZE_MAX, ZCL_ATTR_WRITE_FLAG_NORMAL);
        }
