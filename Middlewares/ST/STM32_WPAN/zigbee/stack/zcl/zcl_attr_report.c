    ...
    LINK_LIST_FOREACH(p, &cluster->reports.list) {
        ...
        if (!send_all && (report->max_interval != ZCL_ATTR_REPORT_MAX_INTVL_CHANGE)) {
            ...
            if (max_intvl_timeout == 0) {
                ...
                ZCL_LOG_PRINTF(zb, __func__, "Will send report [timeout] (cl = 0x%04x (%s), attr = 0x%04x)",
        ...
        do {
            ...
            * Freed at the end of this function. */
            ...
            if (ZbZclAttrRead(cluster, report->attributeId, &attr_type, attr_data, ZCL_ATTRIBUTE_BUFFER_SIZE_MAX, false) != ZCL_STATUS_SUCCESS) {
            ...
            if ((report->dataType >= ZCL_DATATYPE_FLOATING_SEMI)
                ...
                if (!send_all && (delta < report->change)) {
                    break;
                }

                /* ZCL_LOG_PRINTF(zb, __func__, "cl = 0x%04x (%s), attr = 0x%04x, FLOAT",
                    ZbZclClusterGetClusterId(cluster), ZbZclClusterGetDirectionStr(cluster), report->attributeId); */
            ...
            /* Compare minimum change in integer values. */
                ...
                /* ZCL_LOG_PRINTF(zb, __func__, "cl = 0x%04x (%s), attr = 0x%04x, delta = %d, epsilon = %d, ANALOG",
                    ZbZclClusterGetClusterId(cluster), ZbZclClusterGetDirectionStr(cluster),
                    report->attributeId, delta, report->change); */
            ...
            /* Compare changes in digital attributes with hashes (CRC). */
                ...
                if (!send_all && (report->last_value.hash == hash)) {
                    break;
                }
                ZCL_LOG_PRINTF(zb, __func__, "cl = 0x%04x (%s), attr = 0x%04x, hash = 0x%04x",
                    ZbZclClusterGetClusterId(cluster), ZbZclClusterGetDirectionStr(cluster),
            ...
            report->last_report_time = up_time;
#ifdef COND_ZCL_REPORT_VERBOSE_LOGGING
            ZCL_LOG_PRINTF(zb, __func__, "Will send report [%s] (cl = 0x%04x (%s), attr = 0x%04x)",
                send_all ? "send_all" : "change", ZbZclClusterGetClusterId(cluster),
                ZbZclClusterGetDirectionStr(cluster), report->attributeId);
#endif
        } while (false);
...
EXIT_FUNC:
    ...
    if (!disable_periodic_timers) {
        /* If report list is empty, then don't reset timer. */
        if (LINK_LIST_HEAD(&cluster->reports.list) != NULL) {
#ifdef COND_ZCL_REPORT_VERBOSE_LOGGING
            ZCL_LOG_PRINTF(zb, __func__, "Resetting next reporting timeout (cl = 0x%04x (%s), timeout = %d)",
                ZbZclClusterGetClusterId(cluster), ZbZclClusterGetDirectionStr(cluster), next_timeout);
#endif
...
zcl_reporting_create_default_reports(struct ZbZclClusterT *cluster)
{
    ...
    LINK_LIST_FOREACH(p, &cluster->attributeList) {
        ...
        ZCL_LOG_PRINTF(cluster->zb, __func__, "Creating report: cluster = 0x%04x, attr = 0x%04x, min_time = 0x%04x, max_time = 0x%04x, change = %0.3f",
            ZbZclClusterGetClusterId(cluster), attr->info->attributeId, info.default_min_interval, info.default_max_interval, info.change);
