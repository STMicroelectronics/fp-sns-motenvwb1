...
static enum ZclStatusCodeT zcl_otacli_parse_image_block_rsp_success(const uint8_t *payload,
    const unsigned int length, unsigned int *index,
    struct ZbZclOtaImageDefinition *image_definition, struct ZbZclOtaImageData *image_data);
static enum ZclStatusCodeT zcl_otacli_parse_image_block_rsp_wait(const uint8_t *payload,
    const unsigned int length, unsigned int *index,
    struct ZbZclOtaImageWaitForData *image_wait);
...
static enum ZclStatusCodeT zcl_otacli_handle_upgrade_end_rsp(struct ZbZclClusterT *cluster,
    struct ZbZclHeaderT *zcl_header, struct ZbApsdeDataIndT *data_ind);
static void zcl_otacli_handle_image_notify(struct ZbZclClusterT *clusterPtr,
    struct ZbZclHeaderT *zcl_header, struct ZbApsdeDataIndT *data_ind);
...
static enum ZclStatusCodeT ZbZclOtaClientImageUpdateRawCb(struct ZbZclClusterT *clusterPtr,
    uint8_t length, uint8_t *data, void *arg);
static enum ZclStatusCodeT ZbZclOtaClientImageWriteTagCb(struct ZbZclClusterT *clusterPtr,
    struct ZbZclOtaHeader *header,
    uint16_t tag_id, uint32_t tag_length, uint8_t data_length, uint8_t *data, void *arg);
static enum ZclStatusCodeT ZbZclOtaClientImageValidateCb(struct ZbZclClusterT *clusterPtr,
    struct ZbZclOtaHeader *image_definition, void *arg);
static enum ZclStatusCodeT ZbZclOtaClientImageUpgradeEndCb(struct ZbZclClusterT *clusterPtr,
    struct ZbZclOtaHeader *header,
...
static enum ZclStatusCodeT ZbZclOtaClientImageNotifyCb(struct ZbZclClusterT *clusterPtr, uint8_t payload_type,
    uint8_t jitter, struct ZbZclOtaImageDefinition *image_definition, struct ZbApsdeDataIndT *data_ind,
    struct ZbZclHeaderT *zcl_header);
...
ZbZclOtaClientAlloc(struct ZigBeeT *zb, struct ZbZclOtaClientConfig *config, void *arg)
{
    ...
    client = ZbZclClusterAlloc(zb, sizeof(struct cluster_priv_t), ZCL_CLUSTER_OTA_UPGRADE,
            config->endpoint, ZCL_DIRECTION_TO_CLIENT);
    if (client == NULL) {
        return NULL;
    }
    ...
    const uint8_t *input_data, unsigned int input_max_len, void *attr_data, ZclWriteModeT mode, void *app_cb_arg)
{
    ...
    switch (attribute_id) {
            ...
            if ((file_offset != ZCL_INVALID_UNSIGNED_32BIT) && (file_offset != 0U)) {
                ...
                if (file_offset < client->ota_header.header_length) {
                    ZCL_LOG_PRINTF(cluster->zb, __func__,
                        "OTA Client: Error, cannot set file_offset (%d) less than header_length (%d)",
                        file_offset, client->ota_header.header_length);
                    return ZCL_STATUS_INVALID_VALUE;
                }
                if (file_offset > client->ota_header.total_image_size) {
                    ZCL_LOG_PRINTF(cluster->zb, __func__,
                        "OTA Client: Error, cannot set file_offset (%d) greater than total_image_size (%d)",
                ...
                if ((client->config.callbacks.image_validate == NULL)
                    ...
                    ZCL_LOG_PRINTF(cluster->zb, __func__,
                        "OTA Client: Error, cannot set FILE_OFFSET when using default image_validate() callback.");
                    return ZCL_STATUS_FAILURE;
                }
            }
        ...
        case ZCL_OTA_ATTR_IMAGE_UPGRADE_STATUS:
        {
            ...
            ota_status = (enum ZbZclOtaStatus)*input_data;
            if (ota_status > ZCL_OTA_STATUS_WAIT_UPGRADE_EXT) {
                return ZCL_STATUS_INVALID_VALUE;
            }
            if ((mode & ZCL_ATTR_WRITE_FLAG_TEST) == 0U) {
...
ZbZclOtaClientImageTransferStart(struct ZbZclClusterT *cluster)
{
    ...
    /* ZCL_LOG_PRINTF(cluster->zb, __func__, "endpoint 0x%02x data_size 0x%02x",
        ota_cluster->cluster.endpoint, ota_cluster->data_size); */
...
zcl_otacli_match_desc_rsp(struct ZbZdoMatchDescRspT *match_rsp, void *arg)
{
    ...
    ZCL_LOG_PRINTF(cluster->zb, __func__,
        "OTA Server located with EUI64 " LOGFMTx64 ", endpoint 0x%02x",
        LOGVALx64(client->requested_server_ext), client->upgrade_server_endpoint);
...
{
    unsigned int index = 0;
    uint8_t field_control = ZCL_OTA_IMAGE_BLOCK_FC_MIN_PERIOD;
...
static int
zcl_otacli_build_upgrade_end_req(uint8_t *payload, const unsigned int length, uint8_t status,
    struct ZbZclOtaImageDefinition *image_definition)
{
    unsigned int index = 0;
 ...
 */
static enum ZclStatusCodeT
zcl_otacli_parse_query_next_image_rsp(const uint8_t *payload, const unsigned int length,
    struct ZbZclOtaImageDefinition *image_definition,
    uint32_t *image_size)
{
    uint8_t status;
    ...
    struct ZbZclOtaImageWaitForData *image_wait)
{
    ...
    if (zb_zcl_parse_uint16(payload, length, index, &image_wait->minimum_block_period) < 0) {
         ...
         * The client SHALL check the existence of this field by looking at the length
         * of the message. If the field does not exist, then the field SHALL have the value of zero.
         */
...
static enum ZclStatusCodeT
zcl_otacli_parse_image_block_rsp(const uint8_t *payload, const unsigned int length,
    struct ZbZclOtaImageDefinition *image_definition,
...
zcl_otacli_abort_download(struct cluster_priv_t *ota_cluster, enum ZbZclOtaCommandId cmd_id)
{
    ...
    if (status != ZCL_STATUS_SUCCESS) {
        ...
        ZCL_LOG_PRINTF(zb, __func__, "Application returned status = 0x%02x. "
            "Leaving OTA intact for app to resume upon request.", status);
...
zcl_otacli_process_block_data(struct cluster_priv_t *ota_cluster)
{
    ...
    if (ota_cluster->is_ota_header) {
        parsed_header_length = ZbZclOtaHeaderParse(ota_cluster->block_cache,
                (uint8_t)ota_cluster->block_cache_end, &ota_cluster->ota_header);
    ...
    while (true) {
        ...
        if (ota_cluster->field_length_remaining == 0) { /* have not read sub-element header when tag-length is zero */
            if (ota_cluster->block_cache_end > 4) {
                ...
                index = 0;
                zb_zcl_parse_uint32(ota_cluster->block_cache, ota_cluster->block_cache_end,
                    &index, &ota_cluster->field_length_total);
                /* Change the threshold element to 1 byte (minimum amount of data in a image block response)
                 * now that tag-ID and length was receieved */
...
ZbZclOtaClientImageValidateCb(struct ZbZclClusterT *clusterPtr, struct ZbZclOtaHeader *ota_header, void *arg)
{
...
#if 0 /* debugging hash */
    {
        ...
        ZCL_LOG_PRINTF(clusterPtr->zb, __func__, "hash = " LOGFMTx64 LOGFMTx64,
            LOGVALx64(hash_low), LOGVALx64(hash_high));
    }
#endif
...
#else
    ZCL_LOG_PRINTF(clusterPtr->zb, __func__, "Warning, stack was not built with SE components, "
        "so ZbSecEcdsaValidate is not available.");
#endif
    ...
    uint32_t current_time, uint32_t upgrade_time, void *arg)
{
    ...
    activation_policy = (enum ZbZclOtaActivationPolicy)ZbZclAttrIntegerRead(clusterPtr,
            (uint16_t)ZCL_OTA_ATTR_UPGRADE_ACTIVATION_POLICY, NULL, &status);
    if (status != ZCL_STATUS_SUCCESS) {
    ...
    if (upgrade_time == ZCL_OTA_UPGRADE_TIME_WAIT) {
        ZCL_LOG_PRINTF(clusterPtr->zb, __func__, "OTA Client: UpgradeTime equals indefinite wait, "
            "must receive unsolicited UpgradeEndRequest");
